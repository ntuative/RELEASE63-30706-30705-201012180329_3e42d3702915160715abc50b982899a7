package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_923:int = 3;
      
      private var var_408:AssetLoaderStruct;
      
      private var var_921:Timer;
      ...
      private const const_1856:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_408 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_921 = new Timer(this.const_1856,1);
         this.var_921.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_408)
         {
            this.var_408.addEventListener(AssetLoaderEvent.const_30,this.onConfigurationComplete);
            this.var_408.addEventListener(AssetLoaderEvent.const_43,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_408)
         {
            this.var_408.removeEventListener(AssetLoaderEvent.const_30,this.onConfigurationComplete);
            this.var_408.removeEventListener(AssetLoaderEvent.const_43,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_923);
            if(this.var_1403.indexOf("?") > 0)
            {
               _loc2_ = this.var_1403 + "&retry=" + this.var_923;
            }
            else
            {
               _loc2_ = this.var_1403 + "?retry=" + this.var_923;
            ...
            this.var_408 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2556,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1135[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1135[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:AvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1831)
         {
            this.var_1136.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_429[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_429[_loc3_] = _loc6_;
            }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_922)
         {
            _loc7_ = true;
            _loc4_ = this.var_922[_loc3_];
            ...
            if(_loc7_)
            {
               ...
               delete this.var_429[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_922[_loc3_];
         }
         while(_loc6_ < this.var_752.length)
         {
            _loc5_ = this.var_752[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_752.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_921.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_751.length > 0 && this.var_752.length < this.const_1429)
         {
            _loc1_ = this.var_751[0];
            _loc1_.startDownloading();
            this.var_752.push(this.var_751.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_751.indexOf(param1) == -1 && this.var_752.indexOf(param1) == -1)
         {
